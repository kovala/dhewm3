...
==================
*/
void idCommonLocal::SetRefreshOnPrint(bool set) { com_refreshOnPrint = set; }
...
==================
*/
void idCommonLocal::ClearCommandLine(void) { com_numConsoleLines = 0; }
...
void idCommonLocal::CheckToolMode(void) {
  ...
  for (i = 0; i < com_numConsoleLines; i++) {
    ...
    if (!idStr::Icmp(com_consoleLines[i].Argv(0), "renderbump") || !idStr::Icmp(com_consoleLines[i].Argv(0), "editor") || !idStr::Icmp(com_consoleLines[i].Argv(0), "guieditor") || !idStr::Icmp(com_consoleLines[i].Argv(0), "debugger") || !idStr::Icmp(com_consoleLines[i].Argv(0), "dmap") || !idStr::Icmp(com_consoleLines[i].Argv(0), "materialEditor")) {
...
==================
*/
void idCommonLocal::ActivateTool(bool active) { com_editorActive = active; }
...
===============
*/
const char* idCommonLocal::KeysFromBinding(const char* bind) { return idKeyInput::KeysFromBinding(bind); }
...
===============
*/
const char* idCommonLocal::BindingFromKey(const char* key) { return idKeyInput::BindingFromKey(key); }
...
===============
*/
int idCommonLocal::ButtonState(int key) { return usercmdGen->ButtonState(key); }
...
===============
*/
int idCommonLocal::KeyState(int key) { return usercmdGen->KeyState(key); }
...
==================
*/
static void Com_Editor_f(const idCmdArgs& args) { RadiantInit(); }
...
=============
*/
static void Com_EditGUIs_f(const idCmdArgs& args) { GUIEditorInit(); }
...
static void PrintMemInfo_f(const idCmdArgs& args) {
  ...
  f->Printf("total(%s ) image(%s ) model(%s ) sound(%s ): %s\n", idStr::FormatNumber(mi.assetTotals).c_str(), idStr::FormatNumber(mi.imageAssetsTotal).c_str(), idStr::FormatNumber(mi.modelAssetsTotal).c_str(), idStr::FormatNumber(mi.soundAssetsTotal).c_str(), mi.filebase.c_str());
...
==================
*/
static void Com_EditDecls_f(const idCmdArgs& args) { DeclBrowserInit(NULL); }
...
==================
*/
static void Com_EditAFs_f(const idCmdArgs& args) { AFEditorInit(NULL); }
...
==================
*/
static void Com_EditParticles_f(const idCmdArgs& args) { ParticleEditorInit(NULL); }
...
==================
*/
static void Com_EditScripts_f(const idCmdArgs& args) { ScriptEditorInit(NULL); }
...
==================
*/
static void Com_EditPDAs_f(const idCmdArgs& args) { PDAEditorInit(NULL); }
#endif // ID_ALLOW_TOOLS
...
=================
*/
static void Com_Quit_f(const idCmdArgs& args) { commonLocal.Quit(); }
...
=================
*/
void Com_SetMachineSpec_f(const idCmdArgs& args) { commonLocal.SetMachineSpec(); }
...
===============
*/
const idLangDict* idCommonLocal::GetLanguageDict(void) { return &languageDict; }
...
=================
*/
void Com_ReloadLanguage_f(const idCmdArgs& args) { commonLocal.InitLanguageDict(); }
...
=================
*/
void Com_StartBuild_f(const idCmdArgs& args) { globalImages->StartBuild(); }
...
=================
*/
bool idCommonLocal::IsInitialized(void) const { return com_fullyInitialized; }
...
static bool isDemo(void) { return sessLocal.IsDemoVersion(); }
...
idGameCallbacks::idGameCallbacks() : reloadImagesCB(NULL), reloadImagesUserArg(NULL) {}
